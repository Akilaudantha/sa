...
namespace Main_project.Controllers
{
    ...
    public class NewAuctionController : ControllerBase
    {
        ...
        public IActionResult AddNewAuctionItem([FromForm] string name, [FromForm] decimal startingBid, [FromForm] decimal? highestBid, [FromForm] IFormFile image)
        {
            ...
            var uploadsFolder = _environment.WebRootPath ?? Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "uploads");
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    cmd.Parameters.AddWithValue("@HighestBid", highestBid.HasValue ? (object)highestBid.Value : DBNull.Value);
